-- Public barbershop directory (/barberias): lets anonymous visitors search
-- and browse every registered tenant, since tenants RLS otherwise only
-- allows a tenant's own members to read their row.
...
grant execute on function public.public_list_tenants(text) to anon, authenticated;
